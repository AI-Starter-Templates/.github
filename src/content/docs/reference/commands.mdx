...
| With local email catcher | `WITH_MAILPIT=1 ./scripts/compose-up.sh` (dev only) | n/a |
...
| Typecheck + lint + knip | `bun run check` |
...
| Seed superuser (if `SUPERUSER_EMAIL` + `_PASSWORD` set) | `bun run db:seed` |
...
| Lint + format + typecheck + knip | `pnpm check` |
