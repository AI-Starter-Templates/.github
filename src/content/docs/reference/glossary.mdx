...
**The three repos**
The three sibling repositories that compose BoringStack's runtime: `api-template`, `ui-template`, `infra-docker-compose-template`. They sit as siblings under one parent directory because the infra repo references the other two via relative paths in its compose builds. An optional fourth template, `infra-bootstrap-tofu-template`, provisions a VPS via OpenTofu.
