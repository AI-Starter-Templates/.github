...
sequenceDiagram
  ...
  API-->>B: 200 { message } — no cookies, no account, no membership yet
  B->>B: user clicks link in email
  B->>API: POST /auth/verify-email { token }
  API->>DB: UPDATE users SET email_verified_at = now()
  API->>DB: provisionAfterVerification → INSERT app.accounts + auth.account_memberships
