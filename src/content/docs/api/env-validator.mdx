...
|---|---|
| `NODE_ENV=production` + `ALLOWED_ORIGINS` non-empty | Every origin starts with `https://` and contains no wildcards. Empty `ALLOWED_ORIGINS` is valid (same-origin deployment, CORS not mounted). |
| `NODE_ENV=production` | Matching email-provider key(s) non-empty (Cloudflare needs both account ID + token; smtp needs `SMTP_HOST`) |
...
| `NODE_ENV=production` + any Valkey-using feature | `VALKEY_PASSWORD` non-empty |
