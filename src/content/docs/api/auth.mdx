...
Both converge at the same point: `accountsService.provisionAfterVerification` — the single place that creates the personal account + owner membership. **No account exists for an unverified email**, so an abandoned signup never leaves orphan tenant rows in the database.
...
sequenceDiagram
  ...
  API-->>B: 200 { message: "Verification email sent to user@example.com" }
  Note over B,API: NO cookies set; user can't log in yet.
  B->>B: user clicks link in email
  B->>API: POST /auth/verify-email { token }
  API->>DB: UPDATE users SET email_verified_at = now()
  API->>DB: provisionAfterVerification → INSERT accounts + memberships
  API-->>B: 200 + auth_token + refresh_token (idempotent — a double-click is safe)
```
...
- Existing already-verified user adding another OAuth provider → link added, `provisionAfterVerification` is idempotent (returns the existing account + membership).

OAuth refuses to issue a session when the IdP says `emailVerified: false`. The transaction rolls back — no user row, no provider link, no half-state. The caller has to verify through the password flow first.
...
<FaqGroup>
  ...
  <FaqItem title="`EMAIL_NOT_VERIFIED` (403) fires only after correct password">
    Password order on `/login` is: dummy-verify on lookup miss →
    bcrypt → check `email_verified_at`. An attacker who doesn't know
