...
Both converge at the same point: `accountsService.provisionAfterVerification`, the single place that creates the personal account and owner membership. No account exists for an unverified email, so an abandoned signup never leaves orphan tenant rows in the database.
