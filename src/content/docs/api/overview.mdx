...
- src/
  ...
  - api/                Feature folders (auth, users, dashboard, billing, admin, health, notifications)
...
The template ships no example domain feature on purpose. The shipped `auth`, `users`, `billing`, `dashboard`, `admin`, `health`, `notifications` modules are framework. Add your own resources with `bun run new:resource <name>`; the scaffolder writes the four-file anatomy and wires it into `config/routes.ts` so you can't forget a step.
