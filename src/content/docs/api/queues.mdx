...
<DocFileTree
  ...
  nodes={[
    { name: "email-delivery.constants.ts", detail: "Queue name, job name, and retry defaults" },
    { name: "email-delivery.types.ts", detail: "JSON-serializable job-data type" },
    { name: "email-delivery.queue.ts", detail: "BullMQ Queue factory" },
    { name: "email-delivery.worker.ts", detail: "Worker plus structured-logged event handlers" },
    { name: "email-delivery.setup.ts", detail: "Boot wiring for queue + worker" },
  ]}
/>
