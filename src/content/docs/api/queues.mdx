...
| Inline fallback when `QUEUES_ENABLED=false` | Dev and tests boot without Valkey or a separate worker process |
| `removeOnComplete: { age: 1h, count: 100 }` | Bounded Valkey usage without losing recent successes for the dashboard |
