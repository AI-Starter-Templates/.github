---
title: Queues
description: BullMQ on Redis for background jobs. One QueueManager owns lifecycle; dispatch falls back to inline execution when queues are off.
---

import { Aside } from "@astrojs/starlight/components";

Background work uses [BullMQ](https://docs.bullmq.io) over the same Redis the cache lives on. The mental model:

- A **queue** is a named work buffer in Redis.
...
sequenceDiagram
  ...
  Manager->>Redis: ZADD with retry config
  Redis-->>Worker: next job
  Worker->>Worker: process(data)
  alt success
    Worker->>Redis: mark complete (TTL 1h)
  else failure
    Worker->>Redis: schedule retry (exp backoff)
    Note over Redis,Worker: up to 5 attempts, then dead-letter
  end
```
...
| Decision | Reason |
|---|---|
| One `QueueManager` per process | Centralized lifecycle, shutdown, admin stats; new queues can't accidentally skip the retry config |
| Per-queue directory with five small files | Constants + queue + worker + setup + types stay together; producer & consumer can't disagree on names |
| Inline fallback when `QUEUES_ENABLED=false` | Dev and tests boot without Redis or a separate worker process |
| `removeOnComplete: { age: 1h, count: 100 }` | Bounded Redis usage without losing recent successes for the dashboard |
| `removeOnFail: false` | Failed jobs stick around for inspection; manual purge if needed |
...
| File | Holds |
|---|---|
| `<name>.constants.ts` | Queue name, job name, retry defaults; one place so producer + consumer can't drift |
| `<name>.types.ts` | The job-data type. **Strict**: BullMQ serializes to JSON; type-drift here means 3 AM debugging |
| `<name>.queue.ts` | BullMQ `Queue` factory |
| `<name>.worker.ts` | BullMQ `Worker` + structured-logged event handlers |
| `<name>.setup.ts` | Wires queue + worker together at boot |
...
BullMQ retries on failure. If a worker dies *after* writing to the database but *before* marking the job complete, the same job runs again. **Workers must be idempotent.** Patterns:

- **Natural keys.** "Send verification email for `userId=X`, `token=Y`" is idempotent: re-sending the same token is harmless.
- **UNIQUE constraints + catch.** A second `INSERT` of the same audit row fails on the constraint; the worker treats unique-violation as success.
...
1. Create `src/queues/<name>/` with the five-file pattern.
