---
title: Email
description: One pluggable interface, four providers (Cloudflare default, Resend, SendGrid, noop). Templates precompiled to JSON at build time; dispatch is queue-aware.
---
...
3. **Dispatch**: `sendTemplate(...)` chooses queue or inline based on env, so the call site does not care which.

The default is [Cloudflare Email Service](/topics/cloudflare-email/) because it's the cheapest at scale. Resend and SendGrid implementations ship alongside; swapping is a one-env-var change.
...
flowchart LR
  ...
  provider -- cloudflare/resend/sendgrid --> sent[(provider API)]
...
| Decision | Reason |
|---|---|
| Single `IEmailService` interface | Adding Postmark or SES later is one file |
| `noop` provider as fallback when keys are empty | Dev boots without credentials; tests stay deterministic |
| Templates precompiled to JSON at build | No Handlebars parser in the hot path; no template-injection from user data |
| `sendTemplate()` queue-aware, `sendTemplateNow()` inline | Workers (already inside a retry envelope) skip the queue; request handlers go through it |
| Address masking in logs | Email addresses never reach the log pipeline raw |
...
interface IEmailService {
  ...
  readonly providerName: "cloudflare" | "resend" | "sendgrid" | "noop";
}
```
