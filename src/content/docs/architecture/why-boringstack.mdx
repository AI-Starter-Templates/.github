...
BoringStack exists because building products from scratch stopped feeling noble a long time ago.

For years, I kept starting new products by rebuilding the same spine: authentication, sessions, account boundaries, billing hooks, email, background jobs, deployment scripts, observability, env validation, and the little rules that keep a codebase from turning into paste. It was expensive before AI. Now it is also obviously wasteful in tokens, context, and human attention.

BoringStack is the stack I wanted to reach for instead: the boring production parts already composed, documented, and lint-enforced, so the first serious work is the product itself. Fork the templates, boot the system, and spend the same week shipping something real instead of re-deciding where auth, queues, billing, deploys, and API contracts should live.

<DocCallout variant="contract" title="The promise" eyebrow="Why it exists">
  Skip the repeat boilerplate. Keep the architecture. Start product work
  immediately, with AI helping inside a codebase that already has shape.
</DocCallout>
...
<FaqGroup>
  <FaqItem title="Rebuilding the same SaaS spine on every new project" open>
    Auth, sessions, OAuth, email, env validation, audit log, billing hooks,
    queues, all wired and documented.
  </FaqItem>
  <FaqItem title="API and UI drifting apart">
    OpenAPI at `/swagger/json`. `pnpm generate:api` refreshes a typed client.
    Contract drift fails the typecheck.
  </FaqItem>
  <FaqItem title="Architecture slipping during fast iteration (including AI-assisted edits)">
    Custom ESLint plugins encode the shape.
    [`validate`](/architecture/lint-as-contract/) is the merge gate.
  </FaqItem>
  <FaqItem title="Deploy and ops eating weeks before you ship features">
    Compose stack, Traefik TLS, optional [OpenTofu to
    Hetzner](/topics/provisioning-with-tofu/), observability overlays you can
    turn on.
  </FaqItem>
  <FaqItem title="User-facing events with no structure on day one">
    Notification events, dispatch jobs, and channels live in the API. See
