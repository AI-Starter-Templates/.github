...
Each entry below answers four questions: **Choice / Why / What we gave up / When we'd reconsider**. The format is borrowed from the ADR pattern — keep it short, keep it honest, link to the page that uses the choice.

If a "boring" pick on this list ever stops being load-bearing, the entry stops being honest. Update it or drop it.
...
<FaqGroup>
  ...
  <FaqItem title="WUD notify-only image updates">
    **Choice.** [What's Up Docker](/runbooks/image-updates/) overlay reports new image versions to a Discord webhook; it does *not* auto-pull or auto-restart.

    **Why.** Surprise restarts in production cause incidents. A Discord ping plus a dashboard URL gives operator-mediated rollout with effectively zero ongoing effort.

    **What we gave up.** Full automation. That's the point.

    **When we'd reconsider.** When a multi-host fleet makes hand-rollouts uneconomical. By then the host count justifies cluster machinery anyway.
  </FaqItem>
</FaqGroup>
