...
The infra stack is built so the **default** is small (Postgres + Valkey + Traefik + your apps) and everything else; observability, error tracking, queue dashboard, image-update detection; is **opt-in via a flag**. The composition happens in `dev.sh`, which assembles the `docker-compose` invocation based on env vars.

## How a stack is assembled

```mermaid
flowchart LR
  base["base stack<br/>postgres · valkey<br/>traefik · api · ui"]
  stack{"STACK= ?"}
...
`STACK=prod` is mostly the same compose graph with different routing + builds. The data plane (Postgres, Valkey) is identical.
