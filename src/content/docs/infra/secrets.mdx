---
title: Secrets
description: Env-file secrets with filesystem permissions. Pragmatic floor for a single-host stack; rotation playbooks; when to graduate to a real secret manager.
---

import { Aside } from "@astrojs/starlight/components";

The infra stack puts secrets in `compose/.env` (gitignored) and passes them to containers via `env_file:` references. **That's it.** There's no Docker secrets, no Vault, no SOPS; and that's deliberate. For a single-host stack, env files + filesystem permissions are the pragmatic floor.
...
| Decision | Reason |
|---|---|
| `compose/.env` is the single source | One file to back up, one file to protect; no scattered config |
| Gitignored, file mode 600 | Filesystem permissions are the access control |
| `env_file:` references, not inline `environment:` blocks | Avoids leaking secrets into `docker-compose.yml` |
| No vendor secret manager by default | Adding one is harder to undo than to add later |
| Validator-fail-fast on missing required secrets | Misconfiguration shows up at boot, not at the first request |
...
| Category | Examples | Rotation cadence |
|---|---|---|
| Database | `POSTGRES_PASSWORD` | Annually, or after a suspected leak |
| Session | `JWT_SECRET` (≥ 32 chars) | After any incident; otherwise leave alone |
| OAuth | `*_OAUTH_CLIENT_SECRET` | Per provider's policy; rotate after staff turnover |
| Provider keys | `RESEND_API_KEY`, `CLOUDFLARE_EMAIL_API_TOKEN`, `STRIPE_SECRET_KEY` | When a key is leaked; per vendor's rotation guidance |
...
3. Every outstanding session is invalidated. Users get a 401 and re-login.
...
2. **Audit the audit log.** `SELECT * FROM audit.audit_log WHERE created_at > '<leak time>'` and look for anomalies.
3. **Force re-login** by rotating `JWT_SECRET` if session integrity is in doubt.
4. **Check git history.** If the leak was a `.env` accidentally committed: `git log -p compose/.env`; and then assume every secret in it has been compromised.

## When to graduate

Env files + 600 permissions stop being adequate when one of these is true:
...
- **Audit-trail requirements.** Compliance frameworks (SOC 2, HIPAA) want a log of who read which secret when.
- **Automated rotation.** If you need rotation more often than humans can do reliably.

When you cross any of those, common upgrade paths:
