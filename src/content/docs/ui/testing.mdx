...
- **Drift.** Hand-written mocks fall behind the real API shape — tests pass while the real backend doesn't.
- **Mental tax.** Contributors would have to learn the mocking framework *and* the real API.
- **False signal.** "Mock tests are green" is not "the feature works." Only the e2e tier proves the feature.
