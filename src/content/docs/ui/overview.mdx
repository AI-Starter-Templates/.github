...
<FileTree>
  - src/ - app/ App shell: providers, router, main entry - features/ Vertical
  feature folders (auth, dashboard, ...). Add your own. - components/ - ui/
  shadcn/ui primitives - core/ Composed components - global/ App-shell wrappers
  - lib/ - api/ openapi-fetch client + generated schema - env/ Zod-validated
  `import.meta.env` - auth/ OAuth start helper (server-side flow) - logger/
  Structured client logs - i18n/ react-i18next setup + locales - hooks/
  Cross-feature hooks - store/ App-level Zustand stores
</FileTree>

A page or component folder always looks like:

<FileTree>
  - features/dashboard/components/DashboardPage/ - DashboardPage.tsx Pure JSX -
  DashboardPage.hooks.ts All React hooks - DashboardPage.types.ts
  `IDashboardPageView` - DashboardPage.constants.ts - DashboardPage.utils.ts -
  DashboardPage.test.tsx - DashboardPage.stories.tsx - index.ts Re-export
</FileTree>
