...
<p class="bs-diagram-caption">Each UI feature folder splits into role-specific files: a pure-JSX <code>.tsx</code> renders what its hook returns; a <code>.hooks.ts</code> owns all React hooks plus the calls into TanStack Query and Zustand; a <code>.types.ts</code> declares the view-object shape the component reads. Components never touch queries, stores, or env directly.</p>
