...
BoringStack runs on a VPS you control: Docker Compose, GHCR images, Traefik TLS, Cloudflare at the edge. [Why BoringStack](/architecture/why-boringstack/) ties that to the rest of the templates.
...
Push to `main` on `api-template` or `ui-template`. The release workflow builds a Docker image and pushes it to `ghcr.io/<your-owner>/<repo>:latest`. WUD on the VPS detects the new tag and notifies you. You pull and restart with two commands. Traefik handles TLS via Let's Encrypt; Cloudflare proxies all traffic to your single apex domain via same-origin path routing.
