...
<p class="bs-diagram-caption">Both api-template (@sentry/bun) and ui-template (@sentry/react) emit events to the same Sentry-compatible wire protocol. The DSN env var picks the destination: a <code>sentry.io</code> hostname for hosted Sentry, or a <code>glitchtip.localhost</code> hostname for the self-hosted overlay. The SDKs don't know which one they're talking to.</p>
