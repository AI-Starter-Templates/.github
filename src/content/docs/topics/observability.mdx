...
| Bundled exporters: node, postgres, redis | The metrics you'll actually consult during an incident |
...
| Prometheus | internal | 15-day retention |
| Loki | internal | Per-container labels, 7-day retention |
...
| redis-exporter | Redis metrics | Hit ratio, evictions, memory |
| Alertmanager | internal | Routes defined in `alertmanager/` |

Dashboards are pre-provisioned from `compose/grafana/`. Add new ones by dropping JSON files there.
...
The whole overlay runs in ~500MB of memory on the default sizing. Disk grows with retention windows: 15 days of metrics + 7 days of logs is usually under 5 GB for small-to-medium traffic. Adjust retention in `compose/prometheus/prometheus.yml` and `compose/docker-compose.observability.yml`.
