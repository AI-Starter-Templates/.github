...
BoringStack's [Deployment](/topics/deployment/) story is boring: SSH into a VPS, install Docker, clone the infra repo, `compose pull && compose up -d`. That works, and some operators prefer it.
...
A few minutes later the site is live at `https://<your-domain>`: VPS provisioned, DNS configured, HTTPS valid, optional superuser seeded (when `superuser_email` + `superuser_password` are set in `terraform.tfvars`).
...
<FaqGroup>
  ...
  <FaqItem title="Cloudflare API token">
    Cloudflare, My Profile, API Tokens, Custom Token. Scope: `Zone:DNS:Edit`, `Zone:Zone Settings:Edit`, `Zone:Rulesets:Edit` on the target zone.
  </FaqItem>
  ...
  <FaqItem title="OpenTofu binary">
    `brew install opentofu` on macOS, [install docs](https://opentofu.org/docs/intro/install/) elsewhere.
  </FaqItem>
</FaqGroup>
...
# Use the SUPERUSER_EMAIL + SUPERUSER_PASSWORD you set in terraform.tfvars,
# or sign up via the registration form.
```
...
<FaqGroup>
  <FaqItem title="OpenTofu, not Terraform" open>
    Terraform is BUSL-licensed; OpenTofu is the MPL-licensed fork, drop-in compatible. Same `.tf` files work in both.
  </FaqItem>
  ...
  <FaqItem title="Single vps_type variable using provider-native size names">
    `cx32`, `s-2vcpu-4gb`: the same names the provider docs, support, and billing page use.
  </FaqItem>
  <FaqItem title="Opinionated Cloudflare zone defaults">
    SSL strict, HSTS 6mo, TLS min 1.2, browser integrity on. Matches what `production-labels.yml` expects; each setting is one override away.
  </FaqItem>
  <FaqItem title="DNS: apex + www only">
    One A/AAAA pair on the apex serves both the SPA and `/api/*` via same-origin path routing. `www.` is a CNAME to apex with a redirect rule. No `api.` subdomain; Traefik path-routes `/api/*` on the same host.
  </FaqItem>
  ...
  <FaqItem title="Secrets in terraform.tfvars (gitignored)">
    Same pragmatic floor as `compose/.env`; upgrade to a secret manager when team size demands it.
  </FaqItem>
  <FaqItem title="Outputs print, never side-effect">
    Apply prints the IP, ssh command, and site URL. Never auto-opens anything.
  </FaqItem>
  <FaqItem title="Fourth sibling repo, not folded into infra">
...
<FaqGroup>
  ...
  <FaqItem title="bootstrap.sh">
    Versioned shell script: clones the infra repo, renders `compose/.env`, runs `compose pull && compose up -d`.
  </FaqItem>
</FaqGroup>
...
Code updates land via the release workflows. Push to `main` on `api-template` or `ui-template`, a new image tag appears on GHCR, WUD on the VPS posts a notification, then on the VPS:
...
<FaqGroup>
  ...
  <FaqItem title="Site returns 522 from Cloudflare">
    Origin not responding: check `docker compose logs traefik api` on the server.
  </FaqItem>
  ...
  <FaqItem title="Cron backups do not run">
    `rclone config` on the server: the cron entry references a remote that must be configured.
  </FaqItem>
</FaqGroup>
