...
| Decision | Reason |
|---|---|
| rclone, not vendor-specific tools | One config talks to S3, B2, Storj, Wasabi, even your own host via SFTP |
| Gzip locally before upload | Saves bandwidth and storage; pg_dump compresses well |
| `pg_dump`, not file-level snapshots | Logical dumps survive Postgres version upgrades; file-level doesn't |
| Retention enforced remotely | The remote is the source of truth for what exists |
| `BACKUP_DRY_RUN=1` mode | First run prints the plan; promotes to real once you trust it |
| Exits non-zero on any failure | Cron mail capture surfaces failures automatically |
...
| Var | Purpose |
|---|---|
| `POSTGRES_USER`, `POSTGRES_DB` | What to dump |
| `RCLONE_REMOTE_NAME` | The rclone remote alias (configured separately via `rclone config`) |
| `RCLONE_REMOTE_PATH` | Path within the remote |
| `BACKUP_RETENTION_DAYS` | Default 30 |
| `BACKUP_DRY_RUN=1` | Print what would happen; skip side effects |

## Setting it up

1. **Configure rclone once.** `rclone config` on the host. Pick a remote (S3, B2, etc.), name it `backup` (or whatever; match `RCLONE_REMOTE_NAME`).
2. **Copy the wrapper.** `cp scripts/backup-wrapper.example.sh scripts/backup-wrapper.sh && chmod +x scripts/backup-wrapper.sh`.
3. **Dry-run it.** `BACKUP_DRY_RUN=1 ./scripts/backup-wrapper.sh`. Verify the plan reads correctly.
4. **Real run.** Drop the flag, run it once manually, confirm a file lands in the remote.
...
| Cadence | Default retention | What survives |
|---|---|---|
| Daily, 30 days | 30 backups | A month of point-in-time recovery |
| Daily, with monthly retention | Custom; see below | Years of monthly snapshots, days of recent |
...
- **Valkey.** It's cache + queues; not authoritative. If you can't rebuild it, you have an architecture problem, not a backup problem.
- **Audit log table.** It *is* backed up because it's part of Postgres. Just noting it explicitly because some templates treat audit separately.
