...
This runbook guides setup from a fresh Cloudflare account to sending mail via Cloudflare Email Service. It's a single one-time setup; rotations later are just step 4.
...
<p class="bs-diagram-caption">Cloudflare Email setup chain: a Workers Paid plan unlocks the Email Service; enabling it on your domain auto-provisions SPF, DKIM, and DMARC; from there you capture the Account ID (used in the endpoint URL) and a scoped API token (used in the bearer header). The api-template reads both via <code>EMAIL_PROVIDER=cloudflare</code>.</p>
